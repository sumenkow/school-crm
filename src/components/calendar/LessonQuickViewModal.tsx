        ...
        <div className="p-5 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
          <div>
            ...
            <h2 className="text-lg font-bold text-slate-900 leading-snug">
              {activeLesson.groupName}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {t('hero.topic', 'Тема')}: <span className="text-slate-800 font-medium">{activeLesson.topic}</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-2xs cursor-pointer"
              title={t('action.editLesson', 'Изменить параметры урока')}
            >
              <Edit3 className="w-3.5 h-3.5 text-blue-600" />
              <span>{t('action.edit', 'Изменить')}</span>
            </button>
            <button
              onClick={onClose}
